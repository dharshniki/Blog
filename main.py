...
class User(UserMixin, db.Model):
    __tablename__ = "users"
    id = db.Column(db.Integer, primary_key=True)         #A Mixin is simply a way to provide multiple inheritance to Python.
...
class BlogPost(db.Model):
    ...
    author_id = db.Column(db.Integer, db.ForeignKey("users.id"))  #users is table_name of User
...
# you need to remember to copy the original function’s information to the new function. Use functools.wraps()
def admin_only(f):
    @wraps(f)   #wrapper function
...
def register():
    ...
    if request.method == "POST":
        ...
        pass_hash = generate_password_hash(user_pass, method='pbkdf2:sha256', salt_length=8) #used to hash the password
        new_user = User(
